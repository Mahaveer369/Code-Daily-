...
## 2026-08-06 - Array Search in Render Loop
**Learning:** Using array search methods like `.find()` inside an `O(N)` render loop (e.g., `.map()`) can lead to unintended `O(N*M)` performance bottlenecks, especially in components representing large lists like dashboards.
**Action:** Memoize lookup maps with `useMemo` and map keys to fast dictionary lookups to reduce complexity to `O(N+M)`.
